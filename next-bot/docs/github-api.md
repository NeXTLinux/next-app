...
If you want to run a Nextbot App against a GitHub Enterprise instance, you'll need to create and set the `GHE_HOST` environment variable inside of the `.env` file.

```
GHE_HOST=fake.github-enterprise.com
```

When [using Nextbot programmatically](/docs/development.md#run-nextbott-programmatically), set the `baseUrl` option for the [`Nextbot`](https://nextbott.github.io/api/latest/classes/nextbott.Nextbot.html) constructor to the full base Url of the REST API

```js
const MyNextbot = Nextbot.defaults({
