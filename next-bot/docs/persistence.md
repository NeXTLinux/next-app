...
Generally speaking, adding database storage or persistence to your Nextbot App will greatly complicate your life. It's perfectly doable, but for most use-cases you'll be able to manage relevant data within GitHub issues, pull requests and your app's configuration file.
...
Nextbot includes a wrapper for the GitHub API which can enable you to store and manipulate data in the GitHub environment. Since your Nextbot App will usually be running to supplement work done on GitHub, it makes sense to try to keep everything in one place and avoid extra complications.
...
If your Nextbot App needs to store more data than Issues and Pull Requests store normally, you can use the [`nextbott-metadata` extension](/docs/extensions#metadata) to hide data in comments. It isn't meant to be super secure or scalable, but it's an easy way to manage some data without a full database.

There are even more APIs that you can use to increase the functionality of your Nextbot App. You can read about all of the ones available in Nextbot on the [`@octokit/rest` documentation](http://octokit.github.io/rest.js/).
