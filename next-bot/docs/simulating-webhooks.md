...
As you are developing your app, you will likely want to test it by repeatedly triggering the same webhook. You can simulate a webhook being delivered by saving the payload to a file, and then calling `nextbott receive` from the command line.
...
Next, simulate receiving this event being delivered by running:
    ...
    $ node_modules/.bin/nextbott receive -e <event-name> -p <path-to-fixture> <path-to-app>
...
For example, to simulate receiving the `pull_request.labeled` event, run:
    ...
    $ node_modules/.bin/nextbott receive -e pull_request -p test/fixtures/pull_request.labeled.json ./index.js
