...
For example, the [stale](https://github.com/nextbott/stale) app will only scan a target repository for stale issues and pull requests if `.github/stale.yml` exists in that target repository.
...
For example, the [stale](https://github.com/nextbott/stale) app will perform a dry run if there is no `.github/stale.yml` file in the target repository.
...
Any configuration _should_ be stored in the target repository. Unless the app is using files from an established convention, the configuration _should_ be stored in the `.github` directory. See the [API docs for `context.config`](https://nextbott.github.io/api/latest/classes/context.Context.html#config).
...
```yaml
_extends: other/nextbott-settings
other: DDD
```
...
```yaml
_extends: nextbott-settings:.github/other_test.yaml
other: FFF
```
