...
export class Server {
  ...
  constructor(options: ServerOptions = {} as ServerOptions) {
    ...
    this.nextbotApp = new options.Nextbot();
    ...
    this.expressApp.use(
      ...
      createWebhooksMiddleware(this.nextbotApp.webhooks, {
        path: "/",
      })
    );
  ...
  public async start() {
    this.log.info(
      `Running Nextbot v${this.version} (Node.js: ${process.version})`
    );
