...
/**
 *
 * @param appFnOrArgv set to either a nextbot application function: `(app) => { ... }` or to process.argv
 */
export async function run(
  ...
  const serverOptions: ServerOptions = {
    ...
    Nextbot: Nextbot.defaults(nextbotOptions),
  };
  ...
  if (!appId || !privateKey) {
    ...
    // When nextbot is started for the first time, it gets into a setup mode
    ...
    // to anything, as the Nextbot instance is not used in setup it makes no
    // difference anyway.
    server = new Server({
      ...serverOptions,
  ...
  if (Array.isArray(appFnOrArgv)) {
    const pkg = await pkgConf("nextbot");
